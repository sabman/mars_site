    ...
    def self.construct_conditions_from_arguments(attribute_names, arguments)
      ...
      attribute_names.each_with_index do |name, idx| 
        if columns_hash[name].is_a?(SpatialColumn)
          #when the discriminating column is spatial, always use the && (bounding box intersection check) operator : 
          # the user can pass either a geometric object (which will be transformed to a string using the quote method of the database adapter) 
          # or an array representing 2 opposite corners of a bounding box
